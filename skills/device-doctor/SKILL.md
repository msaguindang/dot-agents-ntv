...
## Architecture
- **Inventory:** Resolves friendly device names (e.g., `test-pi`) to IP addresses using the `~/.config/pi/device-inventory.json` file.
...
If the user chooses to update the inventory, use your `edit` or `write` tools to update `~/.config/pi/device-inventory.json`.

---

## Managing the Inventory
If the user asks to "list devices", read `~/.config/pi/device-inventory.json` and present the contents cleanly. If they ask to add a device, prompt them for the `name`, `host` (IP), and `user` (defaulting to key-based auth), then update the JSON file.
